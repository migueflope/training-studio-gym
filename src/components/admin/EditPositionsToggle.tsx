"use client";

import {
  useDraggableButton,
  useDraggableButtons,
} from "@/components/ui/DraggableButtonsContext";
import { DraggableScaleSliderRow } from "@/components/ui/DraggableScaleSlider";
import { Move, Check, AlertCircle, Loader2 } from "lucide-react";

/**
 * Floating admin-only control panel: toggles drag mode and (when active)
 * shows a stack of scale sliders for every button visible in the current
 * section. The toggle itself uses the same drag system, so admins can move
 * the panel out of the way per section.
 */
export function EditPositionsToggle() {
  const { isAdmin, editMode, toggleEditMode, device, section, status } =
    useDraggableButtons();
  const draggable = useDraggableButton("edit_toggle");

  if (!isAdmin) return null;

  const sectionLabel: Record<typeof section, string> = {
    landing: "landing",
    public: "público",
    dashboard: "panel",
    admin: "admin",
  };

  // Only show sliders for buttons that actually render in this section.
  const heroOnly = section === "landing";

  return (
    <div
      ref={draggable.ref}
      style={draggable.style}
      className="fixed top-20 left-3 z-[60] flex flex-col items-start gap-2 pointer-events-none"
    >
      <div className="flex items-center gap-2">
        <button
          type="button"
          onClick={toggleEditMode}
          {...(draggable.dragHandlers ?? {})}
          className={`pointer-events-auto inline-flex items-center gap-2 px-3.5 py-2.5 rounded-full backdrop-blur-md text-xs font-bold transition-all ${
            editMode
              ? "bg-primary text-primary-foreground shadow-[0_0_25px_rgba(212,175,55,0.65)]"
              : "bg-black/80 text-primary border border-primary/40 shadow-[0_0_18px_rgba(212,175,55,0.3)] hover:shadow-[0_0_24px_rgba(212,175,55,0.5)]"
          }`}
        >
          <Move className="w-4 h-4" />
          <span className="hidden md:inline">
            {editMode ? "Soltar para guardar" : "Mover botones"}
          </span>
        </button>
        {editMode && (
          <span className="pointer-events-none inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-black/80 backdrop-blur-md border border-border text-[11px] font-medium text-muted-foreground">
            {device === "mobile" ? "Móvil" : "Escritorio"} · {sectionLabel[section]}
          </span>
        )}
        {status === "saving" && (
          <span className="pointer-events-none inline-flex items-center gap-1 text-[11px] font-medium text-muted-foreground bg-black/70 px-2 py-1 rounded-full backdrop-blur-md">
            <Loader2 className="w-3 h-3 animate-spin" />
            Guardando
          </span>
        )}
        {status === "saved" && (
          <span className="pointer-events-none inline-flex items-center gap-1 text-[11px] font-medium text-success bg-black/70 px-2 py-1 rounded-full backdrop-blur-md">
            <Check className="w-3 h-3" />
            Guardado
          </span>
        )}
        {status === "error" && (
          <span className="pointer-events-none inline-flex items-center gap-1 text-[11px] font-medium text-destructive bg-black/70 px-2 py-1 rounded-full backdrop-blur-md">
            <AlertCircle className="w-3 h-3" />
            Error
          </span>
        )}
      </div>

      {editMode && device === "desktop" && (
        <div className="pointer-events-auto w-72 rounded-xl bg-black/85 backdrop-blur-md border border-primary/30 shadow-[0_0_18px_rgba(212,175,55,0.25)] p-3 space-y-2">
          <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-bold mb-1">
            Tamaño de los botones
          </p>
          <DraggableScaleSliderRow buttonKey="chatbot" label="Chat" />
          {heroOnly && <DraggableScaleSliderRow buttonKey="audio" label="Audio" />}
          {heroOnly && (
            <DraggableScaleSliderRow buttonKey="opacity" label="Opacidad" />
          )}
          <DraggableScaleSliderRow buttonKey="edit_toggle" label="Mover" />
        </div>
      )}
    </div>
  );
}
